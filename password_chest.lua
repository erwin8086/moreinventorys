...
local new_password = function(name, password)
	...
	file:write(minetest.serialize(save))
	file:close()
	file = io.open(minetest.get_worldpath().."/moreinventorys_passwords","a")
	file:write(name.."\n")
	file:close()
end
...
local inv = minetest.create_detached_inventory("moreinventorys_password", {
...
local file = io.open(minetest.get_worldpath().."/moreinventorys_passwords", "r")
if file ~= nil then
